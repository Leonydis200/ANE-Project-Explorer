...
import { AdvancedMetrics, SystemHealth, PerformanceMetrics, OptimizationStatus } from '../types';
  ...
  useEffect(() => {
    const initializeData = async () => {
      try {
        ...
        setError(
          `Failed to initialize system monitor: ${err instanceof Error ? err.message : 'Unknown error'}`
        );
        setIsLoading(false);
      }
    };
    ...
    <ErrorBoundary>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-6">
        ...
        >
          ...
          <div>
            <span
              className={`px-3 py-1 rounded-full ${health?.overall > 80 ? 'bg-success text-white' : health?.overall > 60 ? 'bg-warning text-white' : 'bg-danger text-white'}`}
            >
              {health?.overall ?? 'N/A'}%
            </span>
