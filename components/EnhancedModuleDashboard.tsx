...
export default function EnhancedModuleDashboard() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <Tabs
        tabs={modules.map((m) => ({ title: m.title, value: m.id }))}
        activeTab={tab}
        onChange={setTab}
      >
        ...
        <TabsContent value={tab} activeTab={tab}>
          ...
          >
            <div
              className={`p-4 rounded-lg text-white ${currentModule.color} shadow-lg flex flex-col md:flex-row md:items-center gap-4`}
            >
