...
export default function CyberDashboard() {
  ...
  return (
    <div className="px-3 py-4 sm:px-6 sm:py-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4 lg:gap-6 w-full max-w-7xl mx-auto min-h-screen">
        ...
        fallback={
          <div className="col-span-1 lg:col-span-2 p-3 sm:p-4 bg-red-50 border-l-4 border-red-400 rounded-r-md">
            <p className="text-red-700 text-sm sm:text-base">
              Failed to load system health. Please refresh the page.
            </p>
          </div>
        }
      >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
                  {health?.components ? (
                    Object.entries(health.components).map(([key, value]) => (
                      <div
                        key={key}
                        className="space-y-2 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-150"
                      >
                        <div className="flex justify-between items-center">
                          <span className="text-xs sm:text-sm capitalize font-medium text-gray-700">
      ...
      <ErrorBoundary fallback={<AlertBanner type="error" message="Failed to load alerts." />}>
        <Card className="shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardContent className="h-full p-4 sm:p-6">
            ...
            >
              ...
              {alerts.length > 0 && (
                <span
                  className="ml-2 px-2 py-0.5 text-xs font-semibold rounded-full bg-red-100 text-red-800 min-w-[1.5rem] text-center"
              ...
              {alerts.length > 0 ? (
                <ul className="space-y-3" aria-live="polite">
                  {alerts.map((alert, idx) => (
                    <li
                      key={idx}
                      className="flex items-start p-3 bg-yellow-50 border-l-4 border-yellow-400 rounded-r-md hover:bg-yellow-100 transition-colors duration-150"
                    >
                      <AlertTriangle
                        className="flex-shrink-0 mt-0.5 mr-3 h-4 w-4 text-yellow-600"
                        aria-hidden="true"
                      />
      ...
      <ErrorBoundary
        fallback={<AlertBanner type="error" message="Failed to load system feedback." />}
      >
        <Card className="shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardContent className="h-full p-4 sm:p-6">
      ...
      <ErrorBoundary
        fallback={<AlertBanner type="error" message="Failed to load diagnostics history." />}
      >
        <Card className="col-span-1 lg:col-span-2 shadow-sm hover:shadow-md transition-shadow duration-200">
          <CardContent className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-2 sm:gap-0">
              <h2
                ...
                id="diagnostics-history-heading"
              >
                Diagnostics History
              </h2>
