...
const ModuleSkeleton = ({ isDarkMode }: { isDarkMode: boolean }) => (
  <div className="space-y-6">
    <div className="flex items-center space-x-4">
      <div
        className={`w-14 h-14 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} animate-pulse`}
      />
      <div className="flex-1 space-y-2">
        <div
          className={`h-6 w-3/4 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} animate-pulse`}
        />
        <div
          className={`h-4 w-1/2 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} animate-pulse`}
        />
      </div>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {[1, 2, 3, 4].map((i) => (
        ...
        >
          <div
            className={`h-4 w-1/3 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} animate-pulse mb-2`}
          />
          <div
            className={`h-6 w-2/3 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'} animate-pulse`}
          />
        </div>
      ))}
    </div>
...
const ModuleViewer: React.FC<ModuleViewerProps> = ({ activeModule, isDarkMode }) => {
  ...
  if (isLoading) {
    return (
      <div
        className={`rounded-2xl shadow-xl p-6 transition-colors duration-300 ${isDarkMode ? 'bg-gray-800' : 'bg-white'} border ${borderClass}`}
      >
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex items-start justify-between">
              ...
              >
                <RefreshCw
                  className={`w-5 h-5 ${isLoading ? 'animate-spin' : ''} ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
                />
              </button>
            </div>
            ...
            {module.metrics && Object.keys(module.metrics).length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(module.metrics).map(([key, value]) => (
                  <MetricCard key={key} title={key} value={value} isDarkMode={isDarkMode} />
                ))}
              </div>
            )}
            ...
            {module.lastUpdated && (
              <div
                className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-400'} text-right`}
              >
